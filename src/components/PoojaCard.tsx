...
export const PoojaCard: React.FC<PoojaCardProps> = ({ pooja, onBook }) => {
  ...
  return (
    <div className="group bg-white rounded-2xl border border-stone-200/80 shadow-sm hover:shadow-2xl hover:shadow-amber-900/15 hover:border-amber-500/80 hover:-translate-y-2.5 transition-all duration-500 ease-out overflow-hidden flex flex-col h-full relative">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        <div>
          <div className="flex items-start justify-between gap-2 mb-1">
            <h3 className="font-serif font-bold text-lg text-stone-900 group-hover:text-amber-800 transition-colors duration-300 leading-snug">
              {language === 'hi' && pooja.hindiName ? pooja.hindiName : pooja.name}
            </h3>
          </div>

          {(pooja.hindiName || pooja.name) && (
            <p className="text-xs font-serif text-amber-800 font-medium mb-2 group-hover:text-amber-700 transition-colors">
          ...
          <div className="mt-3 pt-3 border-t border-stone-100 flex items-center justify-between text-xs text-stone-500">
            {pooja.duration && (
              <span className="flex items-center gap-1">
                <Clock className="w-3.5 h-3.5 text-stone-400 group-hover:text-amber-600 transition-colors" />
        ...
        <div className="pt-2 flex items-center gap-2">
          <a
            ...
            className="flex-1 py-2.5 px-3 rounded-xl bg-stone-100 hover:bg-amber-100/70 text-stone-800 hover:text-amber-950 font-semibold text-xs text-center transition-all border border-stone-200 hover:border-amber-300"
          >
